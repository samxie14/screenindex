...
std::vector<std::byte> nsdata_to_bytes(NSData *data) {
    if (!data || data.length == 0) {
        return {};
    }
    const auto *p = static_cast<const std::byte *>(data.bytes);
    return std::vector<std::byte>(p, p + data.length);
}

NSData *cgimage_to_jpeg_data(CGImageRef image, CGFloat quality_0_to_1) {
    NSMutableData *data = [NSMutableData data];
    CGImageDestinationRef dest = CGImageDestinationCreateWithData(
        (__bridge CFMutableDataRef)data, (__bridge CFStringRef)UTTypeJPEG.identifier, 1, nullptr);
    if (!dest) {
        return nil;
    }
    NSDictionary *props = @{
        (__bridge NSString *)kCGImageDestinationLossyCompressionQuality : @(quality_0_to_1)
    };
...
public:
    ...
    std::vector<MonitorInfo> list_monitors() override {
        if (!CGPreflightScreenCaptureAccess()) {
            return {};
        }
